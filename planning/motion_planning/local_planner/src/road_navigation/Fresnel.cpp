...
namespace navigation {
    ...
    void Fresnel::fresnel(double x, double& cos_term, double& sin_term) {
        double xxa;
        double f;
        double g;
        double cc;
        ...
        if (x2 < 2.5625) {
            ...
            sin_term = signum(xxa) * x * x2 * sn / sd;
            cos_term = signum(xxa) * x * cn / cd;
            return;
        }
        if (x > 36974.0) {
            cos_term = signum(xxa)*0.5;
            sin_term = signum(xxa)*0.5;
            return;
        }
        x2 = x*x;
        t = mpi*x2;
        ...
        ss = sin(t);
        t = mpi*x;
        cos_term = 0.5 + (f * ss - g * cc) / t;
        sin_term = 0.5 - (f * cc + g * ss) / t;
        cos_term = cos_term * signum(xxa);
        sin_term = sin_term * signum(xxa);
        return;
    }
    ...
    Fresnel::Fresnel(const Fresnel& orig) {
    }

    Fresnel::~Fresnel() {
    }

    int Fresnel::signum(double x) {
        return x > 0 ? 1 : -1;
    }
}
