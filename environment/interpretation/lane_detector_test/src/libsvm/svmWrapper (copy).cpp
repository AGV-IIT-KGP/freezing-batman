...
int SVM::predictKernelWise(cv::Mat &_input_, int kernel_size){
	//cv::Mat tempImg = _input_.clone();
	for (int i=0;i<_input_.rows;i+=kernel_size) {
		int start_row, start_col;
		int no_cols = _input_.cols;
		...
		for (int j=0;j<_input_.cols;j+=kernel_size) {
			...
			int counter=0;
			for(int r=start_row;r<(start_row+kernel_size);r++){
				for(int c=start_col;c<(start_col+kernel_size);c++){
					x[counter].index = counter;
					x[counter].value = (double)_input_.at<cv::Vec3b>(r,c)[0];
					counter++;
					x[counter].index = counter;
					x[counter].value = (double)_input_.at<cv::Vec3b>(r,c)[1];
					counter++;
					x[counter].index = counter;
					x[counter].value = (double)_input_.at<cv::Vec3b>(r,c)[2];
					counter++;
				}
			}
			x[max_nr_attr].index=-1;
			
			if ( svm_predict(model,x) == 1 ) {
...
/* Note: 
 * Without prediction, ie. just storing the data in the x variable
 * FPS : 176.841
 * Time Elapsed        : 0.00565481
 * 
 * With prediction
 * FPS : 15.7364
 * Time Elapsed        : 0.0635471 if run for entire image
*/
