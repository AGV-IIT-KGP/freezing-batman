...
cv::Mat LaneDetector::colorBasedLaneDetection(cv::Mat frame, int k) {
    ...
    for (int a = 0; a < height; a++) {
        for (int b = 0; b < width; b++) {
            var += ((data[a * gray_frame.step + b] - mean) * (data[a * gray_frame.step + b] - mean)); //widthstep
        }
    }
    var /= (rows * cols);
    std_dev = sqrt(var);
...
void LaneDetector::initializeLaneVariables(int argc, char** argv, ros::NodeHandle node_handle) {
    image_transport(node_handle); //initialize the image_transport from nodeHandle
    ///// ?????????????????????????????????????????????what is N?
...
void LaneDetector::publishLanes(cv::Mat final_img) {
    ...
    image_transport::Publisher pub = image_transport->advertise("camera/image", 10);
}
