...
origin.x= 0, origin.y= 480;  //Wrt top left corner
int count=0;
 geometry_msgs::Pose target_finder(cv::Mat img)
{
    ...
    cv::Canny(img,cdst,25,100,3);
    cv::HoughLinesP(cdst, lines, 1, CV_PI/180, 25, 10, 50);
...
*/
    ...
    p.position.y= -1*target.y+ origin.y;
    p.position.z= center_angle;
    return p;
}
...
void imageCallback(const sensor_msgs::ImageConstPtr msg)
{
  ...
  ros::NodeHandle n;
  ros::Publisher pub_point = n.advertise<geometry_msgs::Pose>("target_point", 50);
  ros::Rate loop_rate(10);
  while (n.ok())
    {
        cv::Mat img;
        geometry_msgs::Pose msge;
        ...
        ROS_INFO("%f %f %f ",msge.position.x, msge.position.y, msge.position.z);
        ros::spinOnce ();
        loop_rate.sleep ();
    }
}
